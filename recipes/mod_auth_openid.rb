...
when 'rhel', 'fedora'
  ...
  bash 'install libopkele' do
    ...
    syslibdir = node['apache2']['lib_dir'][0..node['apache2']['lib_dir'].rindex('/')]
    code <<-EOH
...
version = node['apache2']['mod_auth_openid']['ref']
configure_flags = node['apache2']['mod_auth_openid']['configure_flags']

remote_file "#{Chef::Config['file_cache_path']}/mod_auth_openid-#{version}.tar.gz" do
  source node['apache2']['mod_auth_openid']['source_url']
  mode   '0644'
  action :create_if_missing
end

directory node['apache2']['mod_auth_openid']['cache_dir'] do
...
bash 'install-mod_auth_openid' do
  ...
  #{make_cmd} install
  EOH
  creates "#{node['apache2']['libexecdir']}/mod_auth_openid.so"
  notifies :restart, 'service[apache2]'
  not_if "test -f #{node['apache2']['libexecdir']}/mod_auth_openid.so"
end

template "#{node['apache2']['dir']}/mods-available/authopenid.load" do
