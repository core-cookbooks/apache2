...
if node['apache2']['mod_auth_cas']['from_source']
  ...
  git '/tmp/mod_auth_cas' do
    ...
    revision   node['apache2']['mod_auth_cas']['source_revision']
  ...
  execute 'compile mod_auth_cas' do
    ...
    not_if  "test -f #{node['apache2']['libexecdir']}/mod_auth_cas.so"
  end

  template "#{node['apache2']['dir']}/mods-available/auth_cas.load" do
  ...
  when 'rhel', 'fedora'
    ...
    file "#{node['apache2']['dir']}/conf.d/auth_cas.conf" do
      action :delete
      backup false
    end
  end
...
directory "#{node['apache2']['cache_dir']}/mod_auth_cas" do
