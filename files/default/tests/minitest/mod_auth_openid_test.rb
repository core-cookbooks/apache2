...
describe 'apache2::mod_auth_openid' do
  ...
  it 'installs the opekele library' do
    lib_dir = Pathname.new(node['apache2']['lib_dir']).dirname.to_s
  ...
  it 'does not add the module to httpd.conf' do
    ...
    httpd_config = File.read(File.join(node['apache2']['dir'], conffile))
    refute_match /^LoadModule authopenid_module /, httpd_config
  end

  it 'creates a cache directory for the module' do
    directory(node['apache2']['mod_auth_openid']['cache_dir']).must_exist.with(:owner, node['apache2']['user'])
  end

  it 'ensures the db file is writable by apache' do
    file(node['apache2']['mod_auth_openid']['dblocation']).must_exist.with(:owner, node['apache2']['user']).and(:mode, '644')
  end
