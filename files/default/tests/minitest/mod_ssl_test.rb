...
describe 'apache2::mod_ssl' do
  ...
  it 'does not store SSL config in conf.d' do
    file("#{node['apache2']['dir']}/conf.d/ssl.conf").wont_exist
  end
  ...
  it 'configures SSLCiphersuit from an attribute' do
    assert_match(/^SSLCipherSuite #{node['apache2']['mod_ssl']['cipher_suite']}$/,
      File.read("#{node['apache2']['dir']}/mods-enabled/ssl.conf"))
  end
end
